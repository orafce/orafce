...
-- 'A' HAS NO CAPTURE GROUP AT ALL.
SELECT ORACLE.REGEXP_INSTR('ABC', 'A', 1, 1, 0, '', 0) AS SUBEXPR_0,  -- 1, CORRECT
       ORACLE.REGEXP_INSTR('ABC', 'A', 1, 1, 0, '', 1) AS SUBEXPR_1,  -- 0, CORRECT
       ORACLE.REGEXP_INSTR('ABC', 'A', 1, 1, 0, '', 2) AS SUBEXPR_2;  -- 0, CORRECT

-- REGEXP_SUBSTR() DISAGREES WITH REGEXP_INSTR() FOR THE VERY SAME ARGUMENTS.
SELECT ORACLE.REGEXP_SUBSTR('ABC', 'A', 1, 1, '', 1) IS NULL AS SUBSTR_SAYS_NO_GROUP;

-- WITH A REAL CAPTURE GROUP BOTH BEHAVE CORRECTLY, WHICH SHOWS THE PROBLEM IS
-- CONFINED TO THE "PATTERN HAS NO GROUPS" CASE.
SELECT ORACLE.REGEXP_INSTR('ABC', '(A)', 1, 1, 0, '', 1) AS REAL_GROUP_1,
       ORACLE.REGEXP_INSTR('ABC', '(A)', 1, 1, 0, '', 2) AS REAL_GROUP_2;
