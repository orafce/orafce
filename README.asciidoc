...
Joined paths are canonicalized and checked against the allowed directories.
Parent-directory (`..`) references left by PostgreSQL's canonicalization are
rejected. Use paths without parent references for consistent behavior across
PostgreSQL versions. These checks are lexical, not a filesystem sandbox:
authorized directories and symlinks within them must be controlled by trusted
administrators.
...
Range bounds must be finite. Reversed bounds are swapped; equal bounds return
that value while still advancing the generator. The upper bound remains
exclusive even when the bounds are adjacent floating-point values.
...
Replacements preserve the column type's representation, type modifier, and
domain constraints. A replacement forbidden by a domain raises the normal
constraint error rather than storing an invalid value.
